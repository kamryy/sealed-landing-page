...
export default function ContactPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log("Sending contact message:", request);
              ...
              <div className="relative">
                ...
                <div className="rounded-[22px] border border-white/10 bg-gradient-to-b from-white/[0.04] to-white/[0.01] p-5 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur-sm sm:p-7">
                  <div className="flex flex-col gap-5">
                    ...
                    <div className="group relative">
                      <textarea
                        id="white-list-message"
                        value={message}
                        onChange={(event) => setMessage(event.target.value)}
                        placeholder=" "
                        disabled={isSubmitting}
                        className=" peer h-40 w-full rounded-xl border border-white/15 bg-black/40 px-4 pt-5 pb-1.5 font-dm-sans text-[15px] leading-5 text-white outline-none transition-all placeholder:text-transparent focus:border-sealed-teal focus:shadow-[0_0_0_4px_rgba(107,250,214,0.12)]"
                        required
                      />
                      <label
                        htmlFor="white-list-message"
                        className="pointer-events-none absolute left-4 top-1/6 -translate-y-1/2 font-dm-sans text-sm text-white/45 transition-all duration-150 peer-focus:top-3 peer-focus:text-[11px] peer-focus:text-sealed-teal peer-[&:not(:placeholder-shown)]:top-3 peer-[&:not(:placeholder-shown)]:text-[11px] peer-[&:not(:placeholder-shown)]:text-white/70"
                      >
                        Your message
                      </label>
                    </div>
