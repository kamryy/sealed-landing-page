 ...
 *
 * VENDORED z Sealed indexera (sealed-indexer/src/notifications/ohttp-client.ts).
 * Zgodne bajt-w-bajt z klientem Flutter (sealed_app/lib/remote/ohttp/). NIE
 * RUSZAJ krypto — komentarze niżej dokumentują pułapki, które już kosztowały
 * (eksport sekretu Nk=16 nie 32, układ nagłówka, info-stringi). Każda zmiana
 * tutaj musi iść równolegle ze źródłem, inaczej relay zwraca 400.
 *
 * Uruchamiać TYLKO w Node runtime (używa node:crypto). W route handlerze:
...
async function encapsulateRequest(
  ...
  // response fails to authenticate.
...
// HKDF-Expand (RFC 5869) with empty info length-prefix.
function hkdfExpand(
  prk: Uint8Array,
  info: Uint8Array,
  length: number,
